...
void ChartWidget::Draw(int x, int y, int w, int h) const
{
    if (m_Entries.empty())
    {
        Renderer::RenderText("No results yet", x + 10, y + h / 2 - 8, 19, Color{ 100, 100, 120, 255 });
        return;
    }
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        Renderer::RenderText(buf, (int)(bx - barW), (int)(chartY + chartH - costH - 14), 15, Color{ 150, 220, 255, 255 });

        // Time label
        std::snprintf(buf, sizeof(buf), "%.0fms", e.timeMs);
        Renderer::RenderText(buf, (int)bx, (int)(chartY + chartH - timeH - 14), 15, Color{ 255, 200, 100, 255 });

        // Algorithm name
        int nameW = MeasureText(e.name.c_str(), 10);
        Renderer::RenderText(e.name.c_str(), (int)(bx - nameW / 2.0f + barW * 0.5f), (int)(chartY + chartH + 4), 15, Color{ 180, 180, 200, 255 });
    }

    // Legend
    DrawRectangleV({ (float)(x + w - 90), (float)(chartY) }, { 8, 8 }, Color{ 0, 200, 220, 200 });
    Renderer::RenderText("Cost", x + w - 78, chartY - 1, 15, Color{ 150, 200, 220, 255 });

    DrawRectangleV({ (float)(x + w - 90), (float)(chartY + 14) }, { 8, 8 }, Color{ 255, 160, 50, 200 });
    Renderer::RenderText("Time", x + w - 78, chartY + 13, 15, Color{ 255, 200, 130, 255 });
}
